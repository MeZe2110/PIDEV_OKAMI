...
use App\Service\AjoutNotificationStockService;
...
{                
   ...
   #[Route('/front/{id}', name: 'app_stock_showfront' )]
    ...
    public function index(StockcategoriesRepository $StockcategoriesRepository,StockRepository $stockRepository ,AjoutNotificationStockService $notificationService): Response
   {
    ...
    foreach ($stocks as $stock) {
     if ($stock->getQuantites() < 10) {
            $notificationService->sendEmailIfnewS($stock);
        }
    }
 ...
 public function filtrer(StockcategoriesRepository $StockcategoriesRepository,Request $request){
    ...
    #[Route('/statistique', name: 'app_stockc_statistique', methods: ['GET'])]
    public function statistique(AjoutNotificationStockService $notificationService): Response
   {
    $entityManager = $this->getDoctrine()->getManager();
    $stocks = $entityManager->getRepository(Stock::class)
        ->findBy([], ['nomst' => 'ASC']);
    ...
    return $this->render('stockc/statistique.html.twig', [
        'stocks' => $stocks,
    ]);
